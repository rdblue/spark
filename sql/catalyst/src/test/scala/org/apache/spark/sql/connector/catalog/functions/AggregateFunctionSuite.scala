...
class AggregateFunctionSuite extends SparkFunSuite {
  test("Test simple iavg(int)") {
    ...
    val udaf = bound.asInstanceOf[AggregateFunction[Any, _]]
  ...
  test("Test associative iavg(long)") {
    ...
    val bound = IntegralAverage.bind(new StructType().add("foo", LongType, nullable = false))
    assert(bound.isInstanceOf[PartialAggregateFunction[_, _]])
    val udaf = bound.asInstanceOf[PartialAggregateFunction[Serializable, _]]
...
object IntAverage extends PartialAggregateFunction[(Int, Int), Int] {
  override def name(): String = "iavg"
...
object LongAverage extends PartialAggregateFunction[(Long, Long), Long] {
  override def name(): String = "iavg"
