...
import org.apache.spark.sql.catalyst.expressions.logical.{ApplyTransform, BucketTransform, DateHourTransform, DateTransform, Expression => LogicalExpression, FieldReference, IdentityTransform, LiteralValue, MonthTransform, Transform, YearTransform}
...
class AstBuilder(conf: SQLConf) extends SqlBaseBaseVisitor[AnyRef] with Logging {
  ...
  override def visitTransformList(ctx: TransformListContext): Seq[Transform] = withOrigin(ctx) {
    def getFieldReference(
        ...
        arg: LogicalExpression): FieldReference = {
    ...
    def getSingleFieldReference(
        ctx: ApplyTransformContext,
        arguments: Seq[LogicalExpression]): FieldReference = {
      lazy val name: String = ctx.identifier.getText
      if (arguments.size > 1) {
   ...
   * a value literal.
   */
  override def visitTransformArgument(ctx: TransformArgumentContext): LogicalExpression = {
    withOrigin(ctx) {
